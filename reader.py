...
def load_file_list(input_txt):
    ...
    for i in range(len(lines_input_txt)):
        ...
        if '.jpg' in line_txt or '.png' in line_txt:#读取图片名
            if i != 0:
                num_class += 1
            file_dict[num_class] = []
            file_dict[num_class].append(line_txt)
        if '.jpg' not in line_txt and '.png' not in line_txt:
            if len(line_txt) > 6:#读取框坐标和标签
                split_str = line_txt.split(' ')
                x1_min = float(split_str[0])
                y1_min = float(split_str[1])
                w = float(split_str[2])
                h = float(split_str[3])
                lab_s = ' '.join(split_str[4])#add by ltl添加标签
...
def train_generator(settings, file_list, batch_size, shuffle=True):
    def reader():
        ...
        for item in file_list:
            ...
            if im.mode == 'L' or im.mode == 'RGBA' or im.mode == 'P':
                im = im.convert('RGB')
            ...
            for index_box in range(len(item)):
                if index_box >= 2:#0图片名，1为图片中类别数量
                    ...
                    bbox_sample.append(blabel)####add by ltl自定义标签,标签，因为都是人脸，所以全部为1,占位而已
            ...
            try:# 20210915加入异常判断
                ...
                label = list(sample_labels[:, 0])#[1] * len(face_box)#标签
                batch_out.append((im, face_box, head_box, label))
                if len(batch_out) == batch_size:
                    yield batch_out
                    batch_out = []
            except:
                print('无法读取的文件:',image_path)
