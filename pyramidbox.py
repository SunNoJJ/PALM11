...
def conv_bn(input, filter, ksize, stride, padding, act='soft_relu', bias_attr=False):
...
class PyramidBox(object):
    ...
    def _low_level_fpn(self):
        ...
        def fpn(up_from, up_to):
            ...
            conv1 = fluid.layers.conv2d(
                up_from, ch, 1, act='soft_relu', bias_attr=b_attr)
            ...
            conv2 = fluid.layers.conv2d(
                up_to, ch, 1, act='soft_relu', bias_attr=b_attr)
            if self.is_infer:
                upsampling = fluid.layers.crop(upsampling, shape=conv2)
            # eltwise mul
            conv_fuse = upsampling * conv2
            # conv_fuse = paddle.Tensor.matmul(upsampling, conv2)
            return conv_fuse
    ...
    def _cpm_module(self):
        ...
        def cpm(input):
            ...
            branch2a = conv_bn(input, 256, 1, 1, 0, act='soft_relu')
            branch2b = conv_bn(branch2a, 256, 3, 1, 1, act='soft_relu')
            branch2c = conv_bn(branch2b, 1024, 1, 1, 0, None)
            sum = branch1 + branch2c#The behavior of expression A + B has been unified with elementwise_add(X, Y, axis=-1) from Paddle 2.0.
            ...
            ssh_dimred = fluid.layers.conv2d(
                rescomb, 128, 3, 1, 1, act='soft_relu', bias_attr=b_attr)
            ssh_2 = fluid.layers.conv2d(
                ssh_dimred, 128, 3, 1, 1, bias_attr=b_attr)
            ssh_3a = fluid.layers.conv2d(
                ssh_dimred, 128, 3, 1, 1, act='soft_relu', bias_attr=b_attr)
    ...
    def _l2_norm_scale(self, input, init_scale=1.0, channel_shared=False):
        """
         ...
         Scale(缩放)和Normalization(标准化，规范化，正则化)
        """
    ...
    def _pyramidbox(self):
        ...
        b_attr = ParamAttr(learning_rate=2., regularizer=L2Decay(0.))
        mbox_loc = fluid.layers.conv2d(self.ssh_conv3_norm, 8, 3, 1, 1, bias_attr=b_attr)
        face_loc, head_loc = fluid.layers.split(mbox_loc, num_or_sections=2, dim=1)
        face_loc = permute_and_reshape(face_loc, 4)
        if not self.is_infer:
            head_loc = permute_and_reshape(head_loc, 4)

        mbox_conf = fluid.layers.conv2d(self.ssh_conv3_norm, self.class_number*8, 3, 1, 1, bias_attr=b_attr)
        face_conf3, face_conf1, head_conf3, head_conf1 = fluid.layers.split(
            mbox_conf, num_or_sections=[self.class_number*3, self.class_number*1, self.class_number*3, self.class_number*1], dim=1)
        ...
        face_conf = permute_and_reshape(face_conf, self.class_number+1)
        if not self.is_infer:
            head_conf3_maxin = fluid.layers.reduce_max(head_conf3, dim=1, keep_dim=True)
            head_conf = fluid.layers.concat([head_conf3_maxin, head_conf1], axis=1)
            head_conf = permute_and_reshape(head_conf, self.class_number+1)
        ...
        for i, input in enumerate(inputs):
            ...
            mbox_conf = fluid.layers.conv2d(input, self.class_number*8, 3, 1, 1, bias_attr=b_attr)

            face_conf1, face_conf3, head_conf1, head_conf3 = fluid.layers.split(
                mbox_conf, num_or_sections=[self.class_number*1, self.class_number*3, self.class_number*1, self.class_number*3], dim=1)
            face_conf3_maxin = fluid.layers.reduce_max(face_conf3, dim=1, keep_dim=True)
            face_conf = fluid.layers.concat([face_conf1, face_conf3_maxin], axis=1)

            face_conf = permute_and_reshape(face_conf, self.class_number+1)

            if not self.is_infer:
                head_conf3_maxin = fluid.layers.reduce_max(head_conf3, dim=1, keep_dim=True)
                head_conf = fluid.layers.concat([head_conf1, head_conf3_maxin], axis=1)
                head_conf = permute_and_reshape(head_conf, self.class_number+1)
    ...
    def _vgg_ssd(self):
        ...
        locs, confs = [], []#位置，置信度
        boxes, vars = [], []# 检测网络生成的候选框[Np,4],候选框的方差[Np,4],Np是每个实例的预测总数
        b_attr = ParamAttr(learning_rate=2., regularizer=L2Decay(0.))
