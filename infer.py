...
# 预测图片路径
add_arg('image_path', str, './test_img/1624845776.5983484.jpg', "The image used to inference and visualize.")
# 导出推理模型的位置
...
def infer(args):
    ...
    re_w = image.size[0]
    re_h = image.size[1]
    re_size = min(320.0 / re_w, 320.0 / re_h)  # 画框时需要除掉缩放量
    image = image.resize((int(re_w * re_size), int(re_h * re_size)), Image.LANCZOS)
    ...
    draw_bboxes(image_path, boxs[:, 0:4] / re_size, labels, score, label_name=label_name)
